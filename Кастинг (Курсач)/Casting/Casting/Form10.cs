    ...
    public partial class Form10 : Form
    {
        ...
        private void LoadDataEmployeer()
        {
            try
            {
                ...
                sqlDataAdapter = new MySqlDataAdapter("SELECT *, 'Delete' FROM photograph", sqlConnection);
                ...
                sqlDataAdapter.Fill(dataSet, "photograph");

                dataGridView1.DataSource = dataSet.Tables["photograph"];
        ...
        private void ReloadDataEmployeer()
        {
            try
            {
                ...
                dataSet.Tables["photograph"].Clear();
                ...
                if (e.ColumnIndex == 4)
                {
                    ...
                    if (task == "Delete")
                    {
                        if (MessageBox.Show("Удалить эту строку?", "Удаление", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                        {
                            ...
                            dataSet.Tables["photograph"].Rows[rowIndex].Delete();
                            sqlDataAdapter.Update(dataSet, "photograph");
                        }
                    }
                    else if (task == "Update")
                    {
                        ...
                        dataSet.Tables["photograph"].Rows[r]["name"] = dataGridView1.Rows[r].Cells["name"].Value;
                        dataSet.Tables["photograph"].Rows[r]["surname"] = dataGridView1.Rows[r].Cells["surname"].Value;
                        dataSet.Tables["photograph"].Rows[r]["secondName"] = dataGridView1.Rows[r].Cells["secondName"].Value;

                        sqlDataAdapter.Update(dataSet, "photograph");

                        dataGridView1.Rows[e.RowIndex].Cells[4].Value = "photograph";

                    }
                    else if (task == "Insert")
                    {
                        int rowIndex = dataGridView1.Rows.Count - 2;
                        DataRow row = dataSet.Tables["photograph"].NewRow();
                        ...
                        dataSet.Tables["photograph"].Rows.Add(row);

                        dataSet.Tables["photograph"].Rows.RemoveAt(dataSet.Tables["photograph"].Rows.Count - 1);
